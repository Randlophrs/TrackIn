    ...
    <div class="container mx-auto p-4">
        <div class="flex justify-between items-center">
            ...
            @if (Auth::check() && Auth::user()->is_admin)
                <a href="{{ route('items.create') }}"
                    class="flex items-center gap-4 text-[28px] border-2 border-orange-500 text-orange-500 px-4 py-1 rounded-xl font-semibold">
        ...
        <div class="flex gap-2 mb-4">
            @php
                ...
                $current = request('category', 'semua'); // ambil dari query string, default "semua"
            @endphp
            @foreach ($categories as $category)
                <a href="{{ route('items.index', ['category' => $category === 'semua' ? null : $category]) }}"
                    class="px-4 py-2 rounded-xl font-semibold {{ strtolower($category) === strtolower($current) ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-800' }}">
                    {{ ucfirst($category) }}
                </a>
            @endforeach
        </div>
        ...
        <div class="flex flex-wrap gap-10">
            @foreach ($items as $item)
                @if ($current === 'semua' || strtolower($item->category->name) === strtolower($current))
                    <div class="flex flex-col justify-between border-2 border-blue-500 p-4 rounded-xl w-[23%]">
                        <div>
                            <div class="flex justify-center">
                                <img src="{{ asset($item->image) }}" alt="Image" class="w-24 h-24 object-cover mb-2 rounded-lg">
                            </div>
                        ...
                        <div class="flex justify-between items-center mt-5">
                            ...
                            <a href="" class="text-2xl text-white bg-orange-500 px-8 py-1 font-semibold rounded-xl">Pinjam</a>
                        </div>
                    </div>
                @endif
            @endforeach
